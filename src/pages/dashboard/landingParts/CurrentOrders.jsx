...
const CurrentOrders = ({orders}) => {
  ...
  if(orders) {
    confirmedOrders = orders.filter(order => !order.isFinished && order.isConfirmed && !order.isReady)
  }
  ...
  return (
    ...
    >
      ...
      >
        <h1 className="text-lime-800 font-bold text-3xl">Confirmed Orders</h1>
        {confirmedOrders ? <p className="">{`(${confirmedOrders.length})`}</p> : <p>{`(0)`}</p>}
      </div>
      <div
