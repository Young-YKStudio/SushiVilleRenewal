...
const ALaCarteMenu = ({menu}) => {
  return (
    <section>
      ...
      >
        <p className="font-bold tracking-wide text-2xl text-center text-lime-800">Nigiri</p>
        <p className="font-normal tracking-wide text-xs text-center text-lime-800">{'Sushi, professionally prepared ingredients over rice'}</p>
      </motion.div>
      <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        {menu && menu.map((menus) => {
          if(menus.category === 'A La Carte' && menus.Sub_Category === 'Nigiri' && menus.stock_availability) {
            return <motion.div
              key={menus._id}
              initial={{ opacity: 0.3, y: 15 }}
              whileInView={{ opacity: 1, y: 0, transition: { type: 'spring', bounce: 0.4, duration: 0.8 } }}
      ...
      <motion.div 
        ...
        viewport={{ once: false, amount: 0.8 }}
      >
        <p className="font-bold tracking-wide text-2xl text-center text-lime-800">Sashimi</p>
        <p className="font-normal tracking-wide text-xs text-center text-lime-800">Professionally prepared ingredients</p>
      </motion.div>
      <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        {menu && menu.map((menus) => {
          if(menus.category === 'A La Carte' && menus.Sub_Category === 'Sashimi' && menus.stock_availability) {
            return <motion.div
              key={menus._id}
              initial={{ opacity: 0.3, y: 15 }}
              whileInView={{ opacity: 1, y: 0, transition: { type: 'spring', bounce: 0.4, duration: 0.8 } }}
